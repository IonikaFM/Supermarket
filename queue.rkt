...
(define (stream-reverse L rev)
  (if (null? L)
      rev
      (stream-reverse (cdr L) (stream-cons (car L) rev))
      )
  )

(define (rotate left right Acc)
  (if (stream-empty? left)
      (stream-reverse right Acc)
      (stream-cons (stream-first left) (rotate (stream-rest left) (cdr right) (stream-cons (first right) Acc)))
      )
  )
...
(define (dequeue q)
  (if (> (queue-size-r q) (sub1 (queue-size-l q)))
      (make-queue (stream-rest (rotate (queue-left q) (queue-right q) empty-stream)) '() (sub1 (+ (queue-size-l q) (queue-size-r q))) 0)
